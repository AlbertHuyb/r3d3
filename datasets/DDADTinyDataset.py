...
class DDADTinyDataset(BaseDataset, ABC):
    ...
    def __init__(self, **kwargs):
        super(DDADTinyDataset, self).__init__(**kwargs)
    ...
    def _get_dataset(self) -> Dict[str, List[Group]]:
        ...
        for split in tqdm(['train'], desc='Split'):
            scene_split[split] = []
            dataset_loader = SynchronizedSceneDataset(
                scene_dataset_json=os.path.join(self.path, 'ddad_tiny.json'),
                split=split,
